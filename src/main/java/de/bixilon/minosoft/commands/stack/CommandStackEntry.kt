...
data class CommandStackEntry(
    ...
    fun sign(session: PlaySession, signer: MessageSigner, key: PrivateKey, salt: Long, time: Instant): ByteArray {
        return signer.signMessage(key, data.toString(), null, salt, session.account.uuid, time, LastSeenMessageList(emptyArray()))
    }
}
