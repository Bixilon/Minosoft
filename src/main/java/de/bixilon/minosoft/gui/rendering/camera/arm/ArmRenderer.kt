...
class ArmRenderer(override val context: RenderContext) : Renderer, Drawable {
    ...
    override fun postInit(latch: AbstractLatch) {
        shader.load()
        context.session.events.listen<ResizeWindowEvent> { perspective = CameraUtil.perspective(60.0f.rad, it.size.x.toFloat() / it.size.y, NEAR_PLANE, FALLBACK_FAR_PLANE) }
    }
