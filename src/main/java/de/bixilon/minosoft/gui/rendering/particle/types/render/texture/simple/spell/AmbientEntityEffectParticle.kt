...
class AmbientEntityEffectParticle(session: PlaySession, position: Vec3d, color: RGBColor, data: ParticleData? = null) : SpellParticle(session, position, Vec3d(color.red, color.green, color.blue), data) {
    ...
    companion object : ParticleFactory<AmbientEntityEffectParticle> {
        ...
        override fun build(session: PlaySession, position: Vec3d, velocity: Vec3d, data: ParticleData): AmbientEntityEffectParticle {
            return AmbientEntityEffectParticle(session, position, color = RGBColor(velocity.x, velocity.y, velocity.z), data)
        }
    }
}
