...
interface AbstractLayout<T : Element> : InputElement, DragTarget {
    ...
    fun getAt(position: Vec2f): Pair<T, Vec2f>?
    ...
    override fun onDragMove(position: Vec2f, absolute: Vec2f, draggable: Dragged): Element? {
        val pair = getAt(position)
    ...
    override fun onDragScroll(position: Vec2f, scrollOffset: Vec2f, draggable: Dragged): Element? {
    ...
    companion object {
        ...
        fun Element.getAtCheck(position: MVec2f, element: Element, horizontalAlignments: HorizontalAlignments = HorizontalAlignments.LEFT, modifyY: Boolean = false, size: Vec2f = element.size): Pair<Element, Vec2f>? {
            ...
            return Pair(element, Vec2f(xPosition, position.y))
        }
    }
}
