...
class MoonRenderer(
    ...
    private fun updateUV(phases: MoonPhases) {
        ...
        uvStart = texture.transformUV(Vec2f(1.0f / 4 * coordinates.x, 1.0f / 2 * coordinates.y))
        uvEnd = texture.transformUV(Vec2f(1.0f / 4 * (coordinates.x + 1), 1.0f / 2 * (coordinates.y + 1)))
        meshInvalid = true
    }
