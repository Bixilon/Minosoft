...
class UiArgument() : OptionGroup(), AppliedArgument {
    val headless by option("--headless").flag(default = false)


    override fun apply() {
        ErosOptions.disabled = headless || ErosOptions.disabled
        RenderingOptions.disabled = headless || RenderingOptions.disabled
    }
}
