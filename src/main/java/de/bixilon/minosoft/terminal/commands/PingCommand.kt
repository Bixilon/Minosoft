...
object PingCommand : Command {
    override var node = LiteralNode("ping")
        .addChild(ArgumentNode("address", StringParser(StringParser.StringModes.QUOTED), allowArguments = true) { stack ->
            ...
            val session = StatusSession(address, version)
            session::status.observe(PingCommand) { if (it == null) return@observe; stack.print.print("Received status of §e$address§r: online=§c${it.usedSlots}§r/§a${it.slots}:\n ${it.motd}") }
            session.ping()
