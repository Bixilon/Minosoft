...
class LogPrintStream(
    ...
    override fun print(string: String?) {
        ...
        if (string.startsWith("SLF4J: ") || string.startsWith("SLF4J(W):")) return
        if (string.startsWith("ERROR StatusLogger Log4j2")) return
        if (string.startsWith("Unknown element") && string.endsWith("):")) return

        if (Thread.currentThread().name == "JavaFX-Launcher") return
