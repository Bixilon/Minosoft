...
public class AsyncTaskWorker {
    ...
    public void work(CountUpAndDownLatch progress) {
        LinkedList<Task> doing = (LinkedList<Task>) tasks.clone();
        ...
        doing.parallelStream().forEach((task -> {
            // check if all dependency jobs are complete
            ...
            while (!canStart.get()) {
                ...
                if (!canStart.get()) {
                    try {
                        // wait for any other job to complete
                        latch.waitForChange();
