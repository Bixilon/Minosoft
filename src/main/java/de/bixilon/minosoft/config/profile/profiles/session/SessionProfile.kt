...
package de.bixilon.minosoft.config.profile.profiles.session
...
import de.bixilon.minosoft.config.profile.profiles.session.signature.SignatureC
import de.bixilon.minosoft.config.profile.profiles.session.skin.SkinC
...
class SessionProfile(
    ...
    companion object : ProfileType<SessionProfile> {
        override val identifier = minosoft("session")
        override val clazz = SessionProfile::class.java
        override val icon: Ikon get() = FontAwesomeSolid.NETWORK_WIRED

        override fun create(storage: ProfileStorage?) = SessionProfile(storage)
    }
}
