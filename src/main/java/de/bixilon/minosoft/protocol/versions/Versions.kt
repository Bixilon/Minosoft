...
object Versions : Iterable<Version> {
    ...
    fun register(version: Version) {
        name.put(version.name.lowercase(), version)?.let { throw IllegalStateException("Duplicated version name: ${version.name}") }
    ...
    operator fun get(name: String?): Version? {
        val lowercase = name?.lowercase() ?: return null
        if (lowercase == AUTOMATIC.name) return AUTOMATIC
