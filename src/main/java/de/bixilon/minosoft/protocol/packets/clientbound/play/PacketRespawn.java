...
public class PacketRespawn extends ClientboundPacket {
    ...
    public boolean read(InByteBuffer buffer) {
        if (buffer.getVersionId() < V_20W21A) {
            ...
            this.dimension = buffer.getConnection().getMapping().getDimensionByIdentifier(buffer.readIdentifier());
        } else {
            CompoundTag tag = (CompoundTag) buffer.readNBT();
            this.dimension = buffer.getConnection().getMapping().getDimensionByIdentifier(new ModIdentifier(tag.getStringTag("effects").getValue())); // ToDo
        }
