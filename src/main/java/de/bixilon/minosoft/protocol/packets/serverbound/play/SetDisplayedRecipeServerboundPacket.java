...
public class SetDisplayedRecipeServerboundPacket implements PlayServerboundPacket {
    private final Recipe recipe;

    public SetDisplayedRecipeServerboundPacket(Recipe recipe) {
        this.recipe = recipe;
    }

    @Override
    public void write(OutPlayByteBuffer buffer) {
        buffer.writeString(this.recipe.getResult().getItem().getResourceLocation().getFull());
    }
