...
class EntityPassengerS2CP(buffer: PlayInByteBuffer) : PlayS2CPacket {
    ...
    override fun handle(session: PlaySession) {
        val vehicle = session.world.entities[vehicle] ?: return
        val passengers = passengers.entities(session)
    ...
    override fun log(reducedLog: Boolean) {
        Log.log(LogMessageType.NETWORK_IN, level = LogLevels.VERBOSE) { "Entity passenger (vehicle=$vehicle, passengers=$passengers)" }
    }
}
