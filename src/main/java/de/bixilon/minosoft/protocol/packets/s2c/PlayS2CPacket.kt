...
interface PlayS2CPacket : S2CPacket {
    ...
    fun handle(session: PlaySession) = Unit

    fun check(session: PlaySession) = Unit

    override fun handle(session: Session) {
        if (session !is PlaySession) throw WrongSessionTypeException(PlaySession::class.java, session::class.java)
        check(session)
        handle(session)
    }
}
