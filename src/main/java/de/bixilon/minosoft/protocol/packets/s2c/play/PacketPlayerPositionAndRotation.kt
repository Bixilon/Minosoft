...
import de.bixilon.minosoft.protocol.packets.c2s.play.TeleportConfirmC2SPacket
...
class PacketPlayerPositionAndRotation(buffer: PlayInByteBuffer) : PlayS2CPacket() {
    ...
    override fun handle(connection: PlayConnection) {
        ...
        if (connection.version.versionId >= ProtocolVersions.V_15W42A) {
            connection.sendPacket(TeleportConfirmC2SPacket(teleportId))
        } else {
            connection.sendPacket(PlayerPositionAndRotationC2SPacket(position, rotation, isOnGround))
        }
    }
