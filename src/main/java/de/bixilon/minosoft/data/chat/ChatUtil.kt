...
object ChatUtil {
    ...
    fun PlaySession.getMessageSender(uuid: UUID): MessageSender {
    ...
    fun validateChatMessage(session: PlaySession, message: String) {
        if (message.isBlank()) {
        ...
        if (message.length > session.version.maxChatMessageSize) {
            throw IllegalArgumentException("Message length (${message.length} can not exceed ${session.version.maxChatMessageSize})")
        }
    }
