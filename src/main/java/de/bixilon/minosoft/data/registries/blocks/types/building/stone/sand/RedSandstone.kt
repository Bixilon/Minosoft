...
interface RedSandstone : HardnessBlock {
    ...
    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), RedSandstone {
        ...
        companion object : BlockFactory<Block> {
            override val identifier = minecraft("red_sandstone")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), RedSandstone {
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), RedSandstone {
        override val hardness get() = super<RedSandstone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("red_sandstone_stairs")
