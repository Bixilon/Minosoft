...
interface WaxedCutCopper : Copper {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), WaxedCutCopper {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("waxed_cut_copper")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), WaxedCutCopper {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("waxed_cut_copper_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), WaxedCutCopper {
        override val hardness get() = super<WaxedCutCopper>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("waxed_cut_copper_stairs")
