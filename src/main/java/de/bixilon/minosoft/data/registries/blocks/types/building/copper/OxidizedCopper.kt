...
interface OxidizedCopper : Copper {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), OxidizedCopper {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("oxidized_copper")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.AbstractStoneSlab(identifier, settings), OxidizedCopper {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("oxidized_copper_slab")
