...
interface EndStoneBrick : HardnessBlock {
    ...
    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), EndStoneBrick {
        override val hardness = if (settings.version >= V_1_15) super.hardness else 0.8f
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), EndStoneBrick {
        override val hardness get() = super<EndStoneBrick>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("end_stone_brick_stairs")
