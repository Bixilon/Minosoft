...
interface PolishedBlackstone : Stone {
    override val hardness get() = 2.0f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), PolishedBlackstone {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("polished_blackstone")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), PolishedBlackstone {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("polished_blackstone_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), PolishedBlackstone {
        override val hardness get() = super<PolishedBlackstone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("polished_blackstone_stairs")
