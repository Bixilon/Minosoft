...
interface MossyCobblestone : Cobblestone {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), MossyCobblestone {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("mossy_cobblestone")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), MossyCobblestone {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("mossy_cobblestone_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), MossyCobblestone {
        override val hardness get() = super<MossyCobblestone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("mossy_cobblestone_stairs")
