...
package de.bixilon.minosoft.data.registries.blocks.types.building.end
...
interface Purpur : HardnessBlock {
    override val hardness get() = 1.5f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), Purpur {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("purpur_block")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), Purpur {
        override val hardness get() = 2.0f

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("purpur_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), Purpur {
        override val hardness get() = super<Purpur>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("purpur_stairs")
