...
interface ExposedCopper : Copper {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), ExposedCopper {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("exposed_copper")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.AbstractStoneSlab(identifier, settings), ExposedCopper {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("exposed_copper_slab")
