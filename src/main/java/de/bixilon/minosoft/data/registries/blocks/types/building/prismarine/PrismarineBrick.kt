...
interface PrismarineBrick : HardnessBlock {
    override val hardness get() = 1.5f
    // TODO: Light up

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), PrismarineBrick {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("prismarine_bricks")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), PrismarineBrick {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("prismarine_brick_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), PrismarineBrick {
        override val hardness get() = super<PrismarineBrick>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("prismarine_brick_stairs")
