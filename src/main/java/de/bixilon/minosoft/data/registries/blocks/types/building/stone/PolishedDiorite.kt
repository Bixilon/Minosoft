...
interface PolishedDiorite : Stone {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), FlatteningRenamedModel, PolishedDiorite {
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), PolishedDiorite {
        override val hardness get() = super<Stone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("polished_diorite_stairs")
