...
interface StoneBrick : HardnessBlock {
    override val hardness get() = 2.0f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), StoneBrick {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("stone_brick")
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), StoneBrick {
        override val hardness get() = super<Stone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("stone_brick_stairs")
