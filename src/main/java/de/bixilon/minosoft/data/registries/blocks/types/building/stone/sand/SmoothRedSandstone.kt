...
interface SmoothRedSandstone : HardnessBlock {
    override val hardness get() = 2.0f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), SmoothRedSandstone {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("smooth_red_sandstone")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), SmoothRedSandstone {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("smooth_red_sandstone_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), SmoothRedSandstone {
        override val hardness get() = super<SmoothRedSandstone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("smooth_red_sandstone_stairs")
