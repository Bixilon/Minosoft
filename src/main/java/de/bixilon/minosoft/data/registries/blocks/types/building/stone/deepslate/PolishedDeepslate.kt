...
interface PolishedDeepslate : Stone {
    override val hardness get() = 3.5f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), PolishedDeepslate {

        companion object : BlockFactory<Block> {
            override val identifier = Namespaces.minecraft("polished_deepslate")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), PolishedDeepslate {

        companion object : BlockFactory<Slab> {
            override val identifier = Namespaces.minecraft("polished_deepslate_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), PolishedDeepslate {
        override val hardness get() = super<PolishedDeepslate>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = Namespaces.minecraft("polished_deepslate_stairs")
