...
interface Warped {
    ...
    class Leaves(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : LeavesBlock(identifier, settings), Warped {

        companion object : BlockFactory<Leaves> {
            override val identifier = minecraft("warped_leaves")

            override fun build(registries: Registries, settings: BlockSettings) = Leaves(settings = settings)
        }
    }

    class Door(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : DoorBlock.WoodenDoor(identifier, settings), Warped {

        companion object : BlockFactory<Door> {
            override val identifier = minecraft("warped_door")
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Wooden(identifier, settings), Warped {

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("warped_stairs")
