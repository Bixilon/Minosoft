...
interface DarkPrismarine : HardnessBlock {
    override val hardness get() = 1.5f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), DarkPrismarine {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("dark_prismarine")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), DarkPrismarine {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("dark_prismarine_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), DarkPrismarine {
        override val hardness get() = super<DarkPrismarine>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("dark_prismarine_stairs")
