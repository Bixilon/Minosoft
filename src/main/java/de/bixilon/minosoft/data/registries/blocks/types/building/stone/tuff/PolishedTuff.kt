...
package de.bixilon.minosoft.data.registries.blocks.types.building.stone.tuff
...
interface PolishedTuff : Stone {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), PolishedTuff {

        override val flags get() = super.flags + BlockStateFlags.CAVE_SURFACE

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("polished_tuff")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), PolishedTuff {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("polished_tuff_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), PolishedTuff {
        override val hardness get() = super<PolishedTuff>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("polished_tuff_stairs")
