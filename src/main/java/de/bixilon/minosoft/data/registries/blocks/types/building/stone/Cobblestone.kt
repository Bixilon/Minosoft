...
interface Cobblestone : Stone {
    override val hardness get() = 2.0f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), Cobblestone {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("cobblestone")
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), Cobblestone {
        override val hardness get() = super<Cobblestone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("cobblestone_stairs")
