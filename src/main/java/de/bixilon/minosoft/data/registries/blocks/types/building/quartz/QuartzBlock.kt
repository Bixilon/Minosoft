...
interface QuartzBlock : HardnessBlock {
    ...
    class Slab(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), QuartzBlock {
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), QuartzBlock {
        override val hardness get() = super<QuartzBlock>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("quartz_stairs")
