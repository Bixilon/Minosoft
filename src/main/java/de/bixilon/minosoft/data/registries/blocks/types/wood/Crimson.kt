...
interface Crimson {
    ...
    class Leaves(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : LeavesBlock(identifier, settings), Crimson {

        companion object : BlockFactory<Leaves> {
            override val identifier = minecraft("crimson_leaves")

            override fun build(registries: Registries, settings: BlockSettings) = Leaves(settings = settings)
        }
    }

    class Door(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : DoorBlock.WoodenDoor(identifier, settings), Crimson {

        companion object : BlockFactory<Door> {
            override val identifier = minecraft("crimson_door")

            override fun build(registries: Registries, settings: BlockSettings) = Door(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Wooden(identifier, settings), Crimson {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("crimson_slab")
    ...
    class Fence(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : FenceBlock.Wooden(identifier, settings), Crimson {

        companion object : BlockFactory<Fence> {
            override val identifier = minecraft("crimson_fence")

            override fun build(registries: Registries, settings: BlockSettings) = Fence(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Wooden(identifier, settings), Crimson {

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("crimson_stairs")
