...
interface TuffBrick : Stone {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), TuffBrick {

        override val flags get() = super.flags + BlockStateFlags.CAVE_SURFACE

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("tuff_brick")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), TuffBrick {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("tuff_brick_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), TuffBrick {
        override val hardness get() = super<TuffBrick>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("tuff_brick_stairs")
