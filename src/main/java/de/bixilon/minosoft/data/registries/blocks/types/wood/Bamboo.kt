...
interface Bamboo {
    ...
    class Leaves(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : LeavesBlock(identifier, settings), Bamboo {

        companion object : BlockFactory<Leaves> {
            override val identifier = minecraft("bamboo_leaves")

            override fun build(registries: Registries, settings: BlockSettings) = Leaves(settings = settings)
        }
    }

    class Door(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : DoorBlock.WoodenDoor(identifier, settings), Bamboo {

        companion object : BlockFactory<Door> {
            override val identifier = minecraft("bamboo_door")
    ...
    class Planks(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : PlanksBlock(identifier, settings), Bamboo {
        ...
        companion object : BlockFactory<Planks> {
            override val identifier = minecraft("bamboo_planks")
    ...
    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Wooden(identifier, settings), Bamboo {

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("bamboo_stairs")
