...
interface Andesite : Stone {

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), Andesite {
    ...
    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.AbstractStoneSlab(identifier, settings), Andesite {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("andesite_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), Andesite {
        override val hardness get() = super<Stone>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("andesite_stairs")
