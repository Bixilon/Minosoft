...
class PerVersionEntry<T>(
    val factory: (Int, Any) -> T,
) {
    private lateinit var versions: Map<Int, T>

    fun forVersion(version: Version): T {
        ...
        throw IllegalArgumentException("Can not find an entry for version $version")
    }

    fun initialize(data: Map<String, Any>) {
        check(!this::versions.isInitialized) { "Already initialized!" }

        val versions: SortedMap<Int, T> = sortedMapOf({ t, t2 -> t2 - t })
        for ((versionId, json) in data) {
            val versionId = versionId.toInt()
            versions[versionId] = factory.invoke(versionId, json)
        }
