...
interface DeepslateBricks : Stone {
    override val hardness get() = 3.5f

    open class Block(identifier: ResourceLocation = Companion.identifier, settings: BlockSettings) : RockBlock(identifier, settings), DeepslateBricks {

        companion object : BlockFactory<Block> {
            override val identifier = minecraft("deepslate_bricks")

            override fun build(registries: Registries, settings: BlockSettings) = Block(settings = settings)
        }
    }

    class Slab(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : SlabBlock.Stone(identifier, settings), DeepslateBricks {

        companion object : BlockFactory<Slab> {
            override val identifier = minecraft("deepslate_brick_slab")

            override fun build(registries: Registries, settings: BlockSettings) = Slab(settings = settings)
        }
    }

    class Stairs(identifier: ResourceLocation = this.identifier, settings: BlockSettings) : StairsBlock.Stone(identifier, settings), DeepslateBricks {
        override val hardness get() = super<DeepslateBricks>.hardness

        companion object : BlockFactory<Stairs> {
            override val identifier = minecraft("deepslate_brick_stairs")
