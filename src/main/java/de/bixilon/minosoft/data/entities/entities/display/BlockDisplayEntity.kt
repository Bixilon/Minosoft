...
class BlockDisplayEntity(connection: PlayConnection, entityType: EntityType, data: EntityData, position: Vec3d, rotation: EntityRotation) : DisplayEntity(connection, entityType, data, position, rotation) {
    ...
    companion object : EntityFactory<BlockDisplayEntity> {
        override val identifier: ResourceLocation = minecraft("block_display")
        private val BLOCK_STATE = EntityDataField("BLOCK_STATE")

        override fun build(connection: PlayConnection, entityType: EntityType, data: EntityData, position: Vec3d, rotation: EntityRotation): BlockDisplayEntity {
            return BlockDisplayEntity(connection, entityType, data, position, rotation)
        }
    }
}
