...
class SkyLightPlaceIT {
    ...
    fun `below block 1`() {
        val world = createSession(3, light = true).world
        world[BlockPosition(8, 16, 8)] = StoneTest0.state
        world.assertLight(8, 15, 8, 0xE0)
    }

    fun `below block 2`() {
        val world = createSession(3, light = true).world
        world[BlockPosition(8, 0, 8)] = StoneTest0.state
        world.assertLight(8, -1, 8, 0xE0)
    }

    fun `below block 3`() {
        val world = createSession(3, light = true).world
        world[BlockPosition(8, 15, 8)] = StoneTest0.state
        world.assertLight(8, 14, 8, 0xE0)
    }

    fun `more blocks below block`() {
        val world = createSession(3, light = true).world
        world[BlockPosition(8, 37, 8)] = StoneTest0.state
        for (y in 0..36) {
